...
## Introduction
In order to integrate the Hello World Service into the security mechanisms provided by MSX we need to create two security clients if using MSX 4.2 or below. For MSX 4.3 and above, it is not required to create these security clients [(help me)](../01-msx-developer-program-basics/80-configuring-security-clients#automatic-security-client-creation-for-msx-43-and-up). These clients will enable us to show Swagger documentation, and enforce Role Based Access Control and Tenancy rules for the requests.
